...
def main():
    ...
    with xr.open_dataset(args.input_file) as hgt_anom_ds:

        hgt_anom_ds = hgt_anom_ds.where(
            (hgt_anom_ds[TIME_NAME].dt.year >= START_YEAR) &
            (hgt_anom_ds[TIME_NAME].dt.year <= END_YEAR), drop=True)

        hgt_anom_ds = hgt_anom_ds.where(
            (hgt_anom_ds[LAT_NAME] >= MIN_LATITUDE) &
            (hgt_anom_ds[LAT_NAME] <= MAX_LATITUDE), drop=True)

        clim_base_period = [int(hgt_anom_ds.attrs['base_period_start_year']),
                            int(hgt_anom_ds.attrs['base_period_end_year'])]

        hgt_anom_da = hgt_anom_ds[var_name]

        if args.restrict_to_base_period:
            hgt_anom_da = hgt_anom_da.where(
                (hgt_anom_da[TIME_NAME].dt.year >= clim_base_period[0]) &
                (hgt_anom_da[TIME_NAME].dt.year <= clim_base_period[1]), drop=True)

        kmeans_ds = run_kmeans(
            hgt_anom_da, n_components=args.n_components,
