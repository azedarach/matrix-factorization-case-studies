...
# Run archetypal analysis on HadISST SST anomalies with
# standard settings for algorithm parameters.
#
# Usage: run_hadisst_aa_wrapper.sh <n_components> <delta>
#
# License: MIT
...
PYTHON="python"
RUN_AA="${BIN_DIR}/run_hadisst_aa.py"
...
if test $# -ne 2 ; then
   ...
   echo "Usage: `basename $0` <n_clusters> <delta>"
   exit 1
fi
...
if test "x$CROSS_VALIDATE" = "xyes" ; then
   output_file="${base_filename}.aa.${LAT_WEIGHTS}.k${n_components}.delta$(printf %5.3e $lambda_W).n_init${N_INIT}.n_folds${N_FOLDS}.nc"
   opts="$opts --cross-validate --n-folds $N_FOLDS"
else
   output_file="${base_filename}.aa.${LAT_WEIGHTS}.k${n_components}.delta$(printf %5.3e $lambda_W).n_init${N_INIT}.nc"
fi
