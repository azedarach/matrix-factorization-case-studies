"""
Run archetypal analysis on HadISST SST anomalies.
"""
...
def parse_cmd_line_args():
    ...
    parser = argparse.ArgumentParser(
        description='Run archetypal analysis on HadISST SST anomalies'
    )
...
def run_aa(da, n_components=2, delta=0, lat_weights=LAT_WEIGHTS,
    ...
    """Run archetypal analysis on SST data."""
    ...
    if cross_validate:
        ...
        for train, test in tscv.split(training_data):
            ...
            aa_model = fit_aa_model(
                training_data[train], n_components=n_components, delta=delta,
                init=init, n_init=n_init, tolerance=tolerance,
                max_iterations=max_iterations,
                verbose=verbose, random_state=rng)

            reconstruction = aa_model.inverse_transform(aa_model.weights)

            training_costs.append(aa_model.cost)
            training_rmses.append(
                mean_squared_error(training_data[train], reconstruction,
                                   squared=False))

            test_weights, test_cost = aa_model.transform(training_data[test])

            reconstruction = aa_model.inverse_transform(test_weights)
        ...
        best_model = fit_aa_model(
            training_data, n_components=n_components, delta=delta,
            init=init, n_init=n_init,
        ...
        aa_dictionary_da = xr.DataArray(
            ...
            coords={'component': np.arange(n_components),
                    sample_dim: training_samples},
            dims=['component', sample_dim])
        ...
        archetype_coords['component'] = np.arange(n_components)

        archetype_dims = ['component'] + feature_dims
        ...
        validation_weights, validation_cost = best_model.transform(
            validation_data)
        ...
        aa_ds.attrs['training_set_cost'] = '{:16.8e}'.format(np.mean(test_costs))
        aa_ds.attrs['training_set_cost_std'] = '{:16.8e}'.format(np.std(test_costs))
        aa_ds.attrs['training_set_rmse'] = '{:16.8e}'.format(np.mean(test_rmses))
        aa_ds.attrs['training_set_rmse_std'] = '{:16.8e}'.format(np.std(test_rmses))
        aa_ds.attrs['training_set_size'] = '{:d}'.format(n_training_samples)

        aa_ds.attrs['test_set_cost'] = '{:16.8e}'.format(validation_cost)
        aa_ds.attrs['test_set_size'] = '{:d}'.format(n_validation_samples)
        aa_ds.attrs['test_set_rmse'] = '{:16.8e}'.format(validation_rmse)

        aa_ds.attrs['n_folds'] = '{:d}'.format(n_folds)
        ...
        training_rmse = mean_squared_error(
            training_data, reconstruction, squared=False)
        ...
        aa_ds.attrs['training_set_cost'] = '{:16.8e}'.format(training_cost)
        aa_ds.attrs['training_set_rmse'] = '{:16.8e}'.format(training_rmse)
        aa_ds.attrs['training_set_size'] = '{:d}'.format(n_training_samples)
    ...
    aa_ds.attrs['lat_weights'] = lat_weights
...
def main():
    ...
    with xr.open_dataset(args.input_file) as sst_anom_ds:
        ...
        aa_ds = run_aa(
            sst_anom_da, n_components=args.n_components, delta=args.delta,
